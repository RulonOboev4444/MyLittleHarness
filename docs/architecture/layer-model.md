...
| Automation and analyzer | Diagnostics, project intelligence reports, machine-readable command-surface audit manifest, detach dry-run/apply marker posture, hidden-help tasks/bootstrap diagnostics, explicit package-smoke verification, semantic readiness inspection, bounded no-runtime semantic evaluation, optional preflight warnings/templates, consistency checks, dry-run proposals, no-write snapshot plans, explicit safe apply, bounded state frontmatter repair, bounded docmap repair, create-only AGENTS repair, create-only stable spec repair, and explicit closeout/state writeback | Reports, stdout-only templates, `command_surface` rows for read/write/apply/root/write-path posture, temporary package-smoke workspaces outside the product root, create-only writes, marker-only detach evidence, selected state frontmatter updates, selected AGENTS creation, selected docmap creation, selected stable spec fixture creation, selected docmap route updates, synchronized closeout writeback state and derived active-plan metadata, and snapshot evidence only under scoped product contracts | Analyzer CLI modules |
...
- Optional runtime helpers can refresh disposable generated context or runtime posture only through explicit rails. They have no hidden daemon authority, lock-manager authority, provider gateway authority, lifecycle authority, Git authority, or cache-truth authority.
