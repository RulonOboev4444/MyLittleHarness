...
| Automation and analyzer | Diagnostics, project intelligence reports, detach dry-run/apply marker posture, hidden-help tasks/bootstrap diagnostics, explicit package-smoke verification, semantic readiness inspection, bounded no-runtime semantic evaluation, optional preflight warnings/templates, consistency checks, dry-run proposals, no-write snapshot plans, explicit safe apply, bounded state frontmatter repair, bounded docmap repair, create-only AGENTS repair, and create-only stable spec repair | Reports, stdout-only templates, temporary package-smoke workspaces outside the product root, create-only writes, marker-only detach evidence, selected state frontmatter updates, selected AGENTS creation, selected docmap creation, selected stable spec fixture creation, selected docmap route updates, and snapshot evidence only under scoped product contracts | Analyzer CLI modules |
