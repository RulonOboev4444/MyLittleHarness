...
PreToolUse treats `mylittleharness evidence --record --dry-run|--apply` as the first-class owner route for agent-run evidence refs under `project/verification/agent-runs/*.md`. Direct `apply_patch` or shell writes to those Markdown files remain blocked, and blocked messages point at `evidence --record --dry-run` rather than generic intake when the target is an agent-run evidence record.
...
When an active plan remains open but `phase_status=complete` and `project/project-state.md` carries an MLH closeout/phase writeback marker, PreToolUse may also allow exact Git staging of a coherent route-produced lifecycle set. The set must contain existing lifecycle route files, include state, and include expected companions such as `project/roadmap.md` and `project/archive/plans/*.md` when closeout/writeback points to them; broad add, wildcards, directories, generated/runtime caches, and partial lifecycle sets remain blocked. The block message names either the exact safe staging command for that coherent set or the explicit MLH writeback dry-run route.

Narrow route-produced local commits may pass only when the staged set is coherent and generated/runtime cache paths are absent. `git commit -F <file>` is treated as the message-file option; lowercase `git commit -f <file>` remains a force-style option and is blocked distinctly.
...
- bounded degraded projection summary when `read_projection` uses `detail=auto` on large roots or `detail=degraded`, including source counts, a truncated source sample, cache posture, and next safe refresh commands without rebuilding the full projection
...
`read_projection` accepts optional `detail` (`auto`, `full`, or `degraded`) in addition to per-call `root`. `detail=auto` is the default and may return a bounded degraded packet on large roots instead of rebuilding the full in-memory projection; `detail=full` forces the complete source/link/fan-in projection, and `detail=degraded` always returns the bounded summary. Both full and degraded payloads omit source bodies (`sourceBodyMode = "none"`). The structured payload also carries the compact `agentPacket`, `connectReadiness`, and `mlhd` freshness objects; degraded payloads add `degradedReason`, `sourceSummary`, a truncated `sources` sample, and `boundary.fullProjectionRequest = {"detail": "full"}` so MCP consumers can explicitly ask for the expensive complete view.
...
Inspect mode and explicit full `read_projection` fail open to repo files and the current in-memory projection when generated projection files are missing or stale. Degraded `read_projection` fails soft before full projection rebuild by inspecting only inventory, bounded source rows, and generated-cache marker/presence posture; it remains advisory and points operators at `detail=full`, `mlhd run-once --apply`, `projection --warm-cache --target all`, or direct CLI/source reads when more exact context is needed.
