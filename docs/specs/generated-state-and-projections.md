...
- structured projection-cache posture payloads that report JSON artifacts and SQLite index status, source refs, next safe refresh commands, and a bounded self-heal command such as `mylittleharness --root <root> projection --warm-cache --target all` without refreshing from read-only adapters
