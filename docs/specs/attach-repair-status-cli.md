...
| Hidden compatibility diagnostics | `status`, `validate`, `audit-links`, `context-budget`, `doctor` | Existing read-only diagnostics remain available with their current report names and exit semantics. Normal operators should use bare `check`, `check --deep`, or `check --focus validation|links|context|hygiene` first. |
...
| Closeout/state writeback | `writeback --dry-run`, `writeback --apply` | Explicit hidden MLH-owned closeout/state writeback, Current Focus sync, active-phase body sync, default active-plan archive close, automatic completed-plan archive close, and post-writeback operating-memory compaction. Dry-run is no-write; apply mutates only the state writeback block, selected state lifecycle frontmatter, the marked Current Focus block, matching active-plan derived copies, the canonical active-plan archive move, or safe state-history compaction in live operating roots. |
...
| Plan synthesis write rail | `plan --dry-run`, `plan --apply` | Explicit hidden deterministic active-plan scaffold creation for `project/implementation-plan.md`. Dry-run is no-write; apply is live-operating-root only and writes only the default active plan plus selected `project/project-state.md` lifecycle frontmatter and the marked Current Focus block. |
| Memory lifecycle hygiene rail | `memory-hygiene --dry-run`, `memory-hygiene --apply` | Explicit hidden research/incubation lifecycle cleanup for MLH-owned source artifacts. Dry-run is no-write; apply is live-operating-root only and writes only requested source frontmatter, an explicit reference archive target, and optional exact source-path link repairs. |
...
- `intelligence [--query TEXT] [--search TEXT] [--path TEXT] [--full-text TEXT] [--limit N] [--focus search|warnings|projection|routes]` is a read-only aggregate command. Its default output reports terminal-only sections named `Summary`, `Boundary`, `Drift`, `Repo Map`, `Backlinks`, `Search`, `Fan-In`, and `Projection`.
...
- `intelligence --focus routes` renders compact source inventory plus `Boundary` and `Lifecycle Routes` only. For live operating roots, it reports the same advisory route table as `status`/`check`: `state`, `active-plan`, `incubation`, `research`, `stable-specs`, `decisions`, `verification`, `closeout-writeback`, `archive`, `docs-routing`, and one `future-optional` lane for future verification files, ADRs, and reusable procedural surfaces. For product-source fixtures, fallback/archive roots, generated-output roots, or ambiguous roots, it reports scope instead of live route rows.
...
- `init --apply --project <name>` and compatibility `attach --apply --project <name>` are the implemented mutating attach modes. They are create-only for authority files, refuse product-source compatibility fixtures and fallback/archive or generated-output roots with exit code `2`, refuse symlink or non-directory path conflicts before writing, and may create absent eager scaffold directories plus absent `.codex/project-workflow.toml` and `project/project-state.md` from explicit templates. After successful attach, they build disposable schema v2 JSON projection artifacts and the SQLite FTS/BM25 index under `.mylittleharness/generated/projection/`; SQLite unavailability is warning-only and leaves no current index file.
...
- `writeback --dry-run` is no-write. `writeback --apply` is the implemented MLH-owned closeout/state writeback path and may update existing `project/project-state.md` plus matching active-plan derived copies in the same apply, including the active-phase body status line. With `--archive-active-plan`, or automatically with `--phase-status complete` plus completed closeout facts, the same hidden command is also the bounded completed-plan close rail for the canonical active plan and archive directory. Oversized live operating memory compaction is post-writeback hygiene behind this explicit apply boundary, not a separate command. It is not a repair command and does not make read-only warnings into repair approval.
...
Closeout/state writeback is a separate mutation boundary from repair. `writeback --apply` requires strict readable `project/project-state.md` frontmatter because it writes current closeout facts, lifecycle frontmatter, and the derived Current Focus block rather than recovering missing state structure. It must update all MLH-owned derived active-plan copies it can safely identify during the same apply, so stale `docs_decision: uncertain`, active-plan `status: active`, an active-phase body `- status: pending`, or a stale managed Current Focus block after a completed phase is a diagnostic state, not the expected result of a successful writeback. `writeback --apply --archive-active-plan` closes only the default active-plan lifecycle by moving the active plan into `project/archive/plans/` and clearing active pointers; `writeback --apply --phase-status complete` does the same automatically when closeout facts are complete. Reruns after an inactive archive close are refused/skipped without corrupting files. State auto-compaction runs after any explicit live-root writeback apply that leaves the state file above the threshold, and a compaction refusal or skip is reported as hygiene posture rather than a rollback instruction for the state writeback.
...
Route metadata diagnostics are read-only validation, not repair. `validate`, `check`, and `check --focus validation` may emit `route-metadata-frontmatter`, `route-metadata-status`, `route-metadata-field`, `route-metadata-path`, `route-metadata-destination`, `route-metadata-missing-target`, `route-metadata-stale-reference`, and `route-metadata-authority` for live operating roots. Those findings may guide manual review of `status`, `promoted_to`, `archived_to`, `related_*`, `source_research`, `source_incubation`, `supersedes`, and `superseded_by`, but they do not create a repair proposal, archive action, closeout approval, commit approval, rollback path, or lifecycle decision.
