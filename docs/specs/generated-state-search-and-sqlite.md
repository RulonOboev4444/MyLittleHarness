...
The MCP read/projection adapter exposes the same source-verification posture without taking ownership of generated state. Rootless serving selects the MLH root per tool call instead of baking one target root into client config. `mylittleharness.search` may report exact direct-source matches, projection path/reference matches, and current SQLite FTS/BM25 matches when the index already passes read-only inspection, but the adapter does not refresh missing, dirty, stale, malformed, or corrupt generated caches. `mylittleharness.read_source` returns only a bounded source line slice selected by root-relative path, start line, and limit. `mylittleharness.related_or_bundle` returns nearby projection metadata and graph/link rows without source bodies. These adapter tools are navigation helpers only; deleting generated cache still cannot change lifecycle truth, and cache refresh remains owned by explicit `projection` or `intelligence` paths.
...
The mlhd daemon contract is optional and disabled by default. The implemented `mlhd` control plane may record only disposable runtime markers under `.mylittleharness/runtime/mlhd/` for explicit `status`, `start`, `stop`, and `run-once` actions. `mlhd run-once --dry-run` previews dirty, updating, cooling-down, idle, and warmable projection posture without writes; `mlhd run-once --apply` may invoke the same foreground warm-cache path after the quiet period and then records a disposable `projection-refresh.json` pulse ledger. The refresh remains bounded to `.mylittleharness/generated/projection/`, preserves old-good projection artifacts and SQLite indexes when a publish fails, and degrades to source reads when cache cannot be made current. It must not auto-run from MCP, dashboard, hooks, `check`, or SQLite search, must not create lifecycle or cache-freshness authority, and must keep autostart or supervision behind a later reviewed dry-run/apply install rail.

The connect/readiness action packet is the compact first-contact projection for cache and route posture. Dashboard JSON, check and doctor findings, hook JSON, MCP read_projection payloads, and first-contact reports may expose the packet with lifecycle status, active plan/phase, next legal preview, projection artifact and SQLite status, required versus optional recovery-target counts and examples, docs/docmap posture with role metadata, state-write requirement, one next safe command, deterministic recovery command, and mlhd freshness fields for control status, runtime-cache status, pid status, last tick/action, dirty counts, and last refresh/success/failure timestamps. The packet is a source-bound navigation aid only: it cannot refresh generated cache, make SQLite authoritative, or approve any durable state change, source mutation, VCS action, release action, or provider routing.

Snapshot-protected repair routes that mutate frontmatter, including `lifecycle-markdown-frontmatter-repair` and `spec-posture-frontmatter-repair`, must mark existing generated projection artifacts and SQLite indexes dirty or leave an explicit refresh/recovery command. The cache can accelerate the next read, but the repair target set, planned posture keys, copied pre-repair bytes, and apply evidence remain source-bound route facts, not projection-derived authority.
