...
COMMAND_INTENTS: tuple[CommandIntent, ...] = (
    ...
    CommandIntent(
        "discovery-packet-writer",
        "Write a source-bound pre-plan discovery packet as explicit non-authority project/research evidence.",
        (
            "discovery packet",
            "pre plan discovery",
            "pre-plan discovery",
            ...
            "planning discovery packet",
            "пакет дискавери",
            "дискавери пакет",
            "пакет разведки",
        ),
        "mylittleharness --root <root> discover --dry-run --topic \"<topic>\" --source-ref <project/research/source.md>",
        (
            "mylittleharness --root <root> discover --apply --topic \"<topic>\" --source-ref <project/research/source.md> --quality-status provisional --planning-reliance blocked",
            "mylittleharness --root <root> check",
            "mylittleharness --root <root> roadmap --dry-run --action update --item-id <id> --source-research <project/research/discovery-packet.md>",
        ),
        "live operating root with reviewed source/evidence refs that need a durable discovery packet before planning",
        "discover writes exactly one non-authority project/research packet from explicit operator fields; it cannot run research, choose providers, approve readiness, mutate roadmap/lifecycle state, archive, stage, commit, or open a plan",
    ),
    CommandIntent(
